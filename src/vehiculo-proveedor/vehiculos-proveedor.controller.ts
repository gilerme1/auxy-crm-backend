...
@ApiTags('vehiculos-proveedor')
@ApiBearerAuth()
@Controller('vehiculos-proveedor')
@UseGuards(JwtAuthGuard)
export class VehiculosProveedorController {
  constructor(
    private readonly vehiculosProveedorService: VehiculosProveedorService,
  ) {}
  ...
  create(
    ...
    return this.vehiculosProveedorService.create(dto, {
      id: userId,
      rol,
      proveedorId,
    });
  ...
  @Get(':id/historial')
  @ApiOperation({
    summary: 'Historial de solicitudes atendidas por este vehículo',
  })
  ...
  getHistorial(
    ...
    return this.vehiculosProveedorService.getHistorial(id, {
      rol,
      proveedorId,
    });
  }
  ...
  @Roles(RolUsuario.PROVEEDOR_ADMIN, RolUsuario.SUPER_ADMIN)
  @ApiOperation({ summary: 'Asignar un operador al vehículo' })
  @ApiParam({ name: 'id', description: 'ID del vehículo' })
  @ApiResponse({ status: 201, description: 'Operador asignado exitosamente' })
  @ApiResponse({ status: 400, description: 'Operador no válido' })
  assignOperator(
    @Param('id') id: string,
    ...
    return this.vehiculosProveedorService.assignOperator(id, operadorId, {
      rol,
      proveedorId,
    });
  }

  @Post(':id/unassign-operator')
  @UseGuards(RolesGuard)
  @Roles(RolUsuario.PROVEEDOR_ADMIN, RolUsuario.SUPER_ADMIN)
  @ApiOperation({ summary: 'Desasignar un operador del vehículo' })
  @ApiParam({ name: 'id', description: 'ID del vehículo' })
  @ApiResponse({
    status: 200,
    description: 'Operador desasignado exitosamente',
  })
  unassignOperator(
    @Param('id') id: string,
    @Body('operadorId') operadorId: string,
    ...
    return this.vehiculosProveedorService.unassignOperator(id, operadorId, {
      rol,
      proveedorId,
    });
  }
